    ...
    def __init__(self, gamma, epsilon, lr, input_dims, fc1_dims, fc2_dims, 
        ...
        for junction in junctions:
            self.memory[junction] = {
                ...
                "terminal_memory": np.zeros(self.max_mem, dtype=bool),  # Fixed np.bool to bool
                "mem_cntr": 0,
                "iter_cntr": 0,
            }
...
def run(train=True, model_name="model", epochs=50, steps=500, ard=False):
    ...
    # Debug traffic light configuration
    print("\n=== Traffic Light Configuration ===")
    for tl in all_junctions:
        lanes = traci.trafficlight.getControlledLanes(tl)
        print(f"{tl}: {len(lanes)} lanes - {lanes}")
    ...
    for e in range(epochs):
        try:
            ...
            print(f"Failed to start SUMO in epoch {e}: {e}")
            continue

        print(f"epoch: {e}")

        # Phase definitions for 4 signal groups (adjust based on your traffic light)
        select_lane = [
            ["rrrrGGGG", "rrrryyyy"],  # NS green, EW red
            ["GGGGrrrr", "yyyyrrrr"],  # EW green, NS red
            ["rrrrrrrr", "yyyyyyyy"],  # All red → all yellow
            ["GGGGGGGG", "GGGGGGGG"]   # All green
        ]
                ...
                for junction_number, junction in enumerate(all_junctions):
                    ...
                    if traffic_lights_time[junction] == 0:
                        ...
                        try:
                            ...
                            print(f"Error setting phase for {junction}: {e}")
                            raise
...
def get_options():
    ...
    optParser.add_option("-m", dest='model_name', type='string', default="model", help="name of model")
    optParser.add_option("--train", action='store_true', default=False, help="training or testing")
